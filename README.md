...
Updates (November 2025) Performance and Usability Improvements

Visible Region–Only Detection Users can now run peak detection on just the visible portion of the graph. This significantly improves responsiveness when working with large datasets.

Full Dataset Toggle A new checkbox allows you to choose between analyzing the entire dataset or only the currently zoomed-in view.

Tile Mode for Large Files Files exceeding 5 million points automatically trigger a tile-based loading mode to prevent memory overload and UI freezing.

Threaded Detection Peak detection now runs on a background thread, keeping the interface responsive during computation.

Adjustable Threshold Control When using threshold mode, a numeric input allows for manual tuning of the peak height threshold before analysis.
